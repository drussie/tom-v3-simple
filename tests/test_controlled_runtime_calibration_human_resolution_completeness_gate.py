...
def test_export_human_resolution_completeness_gate_contract_is_pending_safe(
    tmp_path: Path,
) -> None:
    contract_path = tmp_path / "contract.json"

    result = export_controlled_runtime_calibration_human_resolution_completeness_gate_contract(
    ...
    scope = contract["human_resolution_completeness_gate_scope"]
    assert scope["default_state_remains_not_ready_without_explicit_inputs"] is True
...
def test_current_human_resolution_completeness_gate_stays_not_ready(
    tmp_path: Path,
) -> None:
    paths = _paths(tmp_path)
    _export_gate_contract(paths)
    ...
    assert inputs["source_bp71_human_resolution_record_status"] == (
        "human_resolution_record_pending_explicit_inputs"
    )
    assert inputs["source_bp71_human_resolution_provided_status"] == (
        "human_resolution_not_provided"
    )
    assert inputs["source_bp71_human_resolution_completeness_status"] == (
    ...
    inputs_validation = (
        validate_controlled_runtime_calibration_human_resolution_completeness_gate_inputs(
            contract_path=paths["contract"],
            completeness_gate_inputs_path=paths["inputs"],
    ...
    assert gate_result["human_resolution_completeness_gate_status"] == (
        "human_resolution_completeness_gate_not_ready"
    )
    assert gate_result["operator_input_completeness_status"] == "operator_inputs_incomplete"
    assert gate_result["candidate_input_completeness_status"] == "candidate_inputs_incomplete"

    gate = _read(paths["gate"])
    assert gate["missing_input_status"] == "required_human_inputs_missing"
    assert gate["runtime_application_status"] == "not_executed"
    assert gate["runtime_config_changed"] is False
    assert gate["mutation_status"] == "no_runtime_mutation_due_to_blocker"
    assert gate["production_config_status"] == "not_created"
    assert gate["baseline_update_status"] == "not_replaced"
    assert gate["model_update_status"] == "not_modified"
    assert gate["missing_input_matrix"]["missing_input_count"] >= 9
    missing_fields = {
        entry["field"]
        for entry in gate["missing_input_matrix"]["entries"]
    ...
    assert gate["final_gate_rerun_readiness"]["final_gate_rerun_readiness_status"] == (
        "final_gate_rerun_not_ready_missing_human_resolution"
    )
    assert gate["reexecution_readiness"]["reexecution_readiness_status"] == (
        "reexecution_not_ready_blockers_unresolved"
    )
    assert gate["non_claims"] == NON_CLAIMS
    assert not (
        FORBIDDEN_HUMAN_RESOLUTION_COMPLETENESS_GATE_TOKENS
        & _walk_exact_strings_and_keys(gate)
    )

    gate_validation = validate_controlled_runtime_calibration_human_resolution_completeness_gate(
        contract_path=paths["contract"],
        human_resolution_completeness_gate_path=paths["gate"],
        output_path=paths["gate_validation"],
        validated_at=datetime(2026, 6, 21, 2, 5, tzinfo=UTC),
    )
    assert gate_validation["ok"] is True
    assert gate_validation["status"] == "valid"
...
def test_missing_human_resolution_record_blocks_completeness_gate(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    _export_gate_contract(paths)

    inputs_result = build_controlled_runtime_calibration_human_resolution_completeness_gate_inputs(
        contract_path=paths["contract"],
        source_explicit_human_resolution_record_path=tmp_path / "missing-record.json",
        output_path=paths["inputs"],
    )
    assert inputs_result["ok"] is True
    assert inputs_result["human_resolution_completeness_gate_status"] == (
    ...
    assert gate_result["operator_input_completeness_status"] == "operator_inputs_missing"
    assert gate_result["candidate_input_completeness_status"] == "candidate_inputs_missing"
    assert gate_result["runtime_config_changed"] is False
    assert gate_result["mutation_status"] == "no_runtime_mutation_due_to_blocker"


def test_complete_future_human_resolution_gate_allows_only_final_gate_rerun(
    tmp_path: Path,
) -> None:
    paths = _paths(tmp_path)
    _export_gate_contract(paths)
    ...
    assert gate_result["next_action_recommendation"] == [
        "rerun_final_gate_after_human_resolution",
        "prepare_final_gate_rerun_blueprint",
    ]

    gate = _read(paths["gate"])
    assert gate["missing_input_matrix"]["missing_input_count"] == 0
    assert gate["human_resolution_completeness_status"] == "human_resolution_complete"
    assert gate["missing_input_status"] == "no_required_inputs_missing"
    assert gate["runtime_application_status"] == "not_executed"
    assert gate["runtime_config_changed"] is False
    assert gate["mutation_status"] == "no_runtime_mutation_due_to_blocker"
    assert gate["production_config_status"] == "not_created"
    assert gate["baseline_update_status"] == "not_replaced"
    assert gate["model_update_status"] == "not_modified"
    assert gate["reexecution_readiness_status"] == "reexecution_blocked_final_gate_not_rerun"
...
def _export_gate_contract(paths: dict[str, Path]) -> None:
    result = export_controlled_runtime_calibration_human_resolution_completeness_gate_contract(
