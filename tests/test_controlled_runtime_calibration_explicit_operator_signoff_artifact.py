...
def test_export_explicit_operator_signoff_artifact_contract_is_pending_safe(
    tmp_path: Path,
) -> None:
    ...
    assert contract["signoff_artifact_scope"][
        "default_state_remains_pending_without_explicit_operator_input"
    ] is True
    assert contract["signoff_artifact_scope"]["does_not_fabricate_operator_signoff"]
    assert contract["signoff_artifact_scope"]["does_not_select_candidate"]
    assert contract["signoff_artifact_scope"]["does_not_rerun_final_gate"]
    assert contract["signoff_artifact_scope"]["does_not_execute_application"]
    assert contract["signoff_artifact_scope"]["does_not_modify_model_weights"]
    assert contract["signoff_artifact_scope"]["does_not_replace_baselines"]
...
def test_current_explicit_operator_signoff_artifact_stays_pending(
    tmp_path: Path,
) -> None:
    ...
    assert inputs["selected_candidate_config_ref"] is None
    assert inputs["non_claims"] == NON_CLAIMS
    assert not (
        FORBIDDEN_EXPLICIT_OPERATOR_SIGNOFF_TOKENS & _walk_exact_strings_and_keys(inputs)
    )

    inputs_validation = (
        validate_controlled_runtime_calibration_explicit_operator_signoff_artifact_inputs(
            contract_path=paths["contract"],
            signoff_inputs_path=paths["inputs"],
    ...
    artifact_result = build_controlled_runtime_calibration_explicit_operator_signoff_artifact(
        contract_path=paths["contract"],
        signoff_inputs_path=paths["inputs"],
    ...
    assert artifact_result["signoff_artifact_status"] == (
    ...
    assert artifact["operator_identity_ref"] is None
    assert artifact["operator_signoff_timestamp"] is None
    assert artifact["operator_attestation_text"] is None
    ...
    assert artifact["production_config_status"] == "not_created"
    assert artifact["baseline_update_status"] == "not_replaced"
    assert artifact["model_update_status"] == "not_modified"
    assert artifact["signoff_readiness"]["readiness_checks"][
        "operator_signoff_explicitly_recorded"
    ] is False
    assert artifact["signoff_readiness"]["readiness_checks"][
        "selected_candidate_explicitly_recorded"
    ] is False
    assert artifact["operator_attestation_template"]["required_operator_fields"][
    ...
    artifact_validation = (
        validate_controlled_runtime_calibration_explicit_operator_signoff_artifact(
            contract_path=paths["contract"],
            signoff_artifact_path=paths["artifact"],
    ...
    requirements = build_controlled_runtime_calibration_operator_signoff_requirements_report(
        contract_path=paths["contract"],
        signoff_artifact_path=paths["artifact"],
    ...
    template = build_controlled_runtime_calibration_operator_attestation_template(
        contract_path=paths["contract"],
        signoff_artifact_path=paths["artifact"],
        output_path=paths["attestation"],
        generated_at=datetime(2026, 6, 21, 3, 5, tzinfo=UTC),
    )
    assert template["ok"] is True
    assert template["operator_attestation_status"] == "operator_attestation_required"

    readiness = build_controlled_runtime_calibration_operator_signoff_readiness_report(
        contract_path=paths["contract"],
        signoff_artifact_path=paths["artifact"],
...
def test_incomplete_explicit_operator_signoff_is_rejected(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    _export_contract(paths)
    ...
    result = (
        build_controlled_runtime_calibration_explicit_operator_signoff_artifact_inputs(
            contract_path=paths["contract"],
            explicit_operator_signoff_ref="operator-signoff-ref",
            output_path=paths["inputs"],
        )
    )
