...
- Current phase: Blueprint 43 complete; gameplay gate regression baseline v1 added
- Current goal: Preserve the BP22-BP36 structural expansion freeze, keep protected regression
  ...
  fixture-safe structural regression baseline without creating tennis truth
...
- Local TOM v1 asset policy: model files under `model_assets/tom_v1/` remain ignored local assets; `best_ball_v2_1280.pt`, `yolo26x.pt`, `yolo26n.pt`, and `yolo26s.pt` are candidate inputs for existing real detection smoke; `yolo26x-pose.pt` is a candidate input for existing real pose smoke; `view_classifier_gameplay.pt` is inspected/hashed by the Blueprint 38 gameplay segment gate and Blueprint 43 regression baseline verifier; `keypoints_model.pth` still requires a future TOM v1-specific adapter
